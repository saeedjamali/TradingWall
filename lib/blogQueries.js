...
export async function getPublicPosts({ category, tag, q, page = 1, limit = 12 } = {}) {
  await connectDB()
  ...
  const [items, total] = await Promise.all([
    BlogPost.find(filter)
      .select('title slug excerpt coverImage coverImageAlt category tags publishedAt views ratingSum ratingCount authorName')
      .sort({ publishedAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean(),
