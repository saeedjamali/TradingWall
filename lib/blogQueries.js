...
export async function getPinnedPosts({ category, tag, q, limit = 3 } = {}) {
  ...
  const conditions = []
  if (category) {
    conditions.push({ $or: [{ category }, { categories: category }] })
  }
  ...
  const items = await BlogPost.find(filter)
    .select('title slug excerpt coverImage coverImageAlt category categories tags publishedAt views ratingSum ratingCount authorName isPinned pinPriority')
...
export async function getPublicPostBySlug(slug, { countView = false } = {}) {
  ...
  const related = await BlogPost.find({
    ...
    $or: [
      ...
      .select('title slug excerpt coverImage category categories publishedAt isPinned')
      .sort(PUBLIC_POST_SORT)
    .limit(3)
    .lean()
